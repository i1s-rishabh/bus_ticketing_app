...
const bookTickets = async (req, res) => {
  ...
  try {
    ...
    if (isOutOfRange(seats_no)) {
      return res.json({ msg: "No such seats are available in the bus" });
    }
...
const isOutOfRange = (selected_seats) => {
  var flag = true;
  for (let i = 0; i < selected_seats.length; i++) {
    for (let j = 0; j < bus_size.length; j++) {
      if (bus_size[j].includes(selected_seats[i])) {
        flag = false;
        break;
      } else {
        flag = true;
